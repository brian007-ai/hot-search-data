...
// 央视特殊处理：从 JS 变量 contentdate 提取正文
function extractCctvContent(html) {
  if (!html) return ''
  // 尝试从 contentdate 变量提取（处理转义引号：style=\"...\"）
  const patterns = [
...
async function enrichContent(platform, items) {
  ...
  const results = await mapWithConcurrency(top, DETAIL_CONCURRENCY, async (it) => {
    ...
    // ---- 央视：从 JS 变量 contentdate 提取正文 ----
    if (platform === 'cctv') {
      if (!it.url) return { content: '' }
      try {
        const r = await fetch(it.url, { timeout: DETAIL_TIMEOUT })
        let content = extractCctvContent(r.data || '').slice(0, CONTENT_MAX_LEN)
    ...
    try {
      const r = await fetch(it.url, { timeout: DETAIL_TIMEOUT })
      let content = extractContentFromHtml(r.data || '', platform).slice(0, CONTENT_MAX_LEN)
